...
Executes side-by-side player comparisons across 1-GW EV and 4-GW Horizon xP.
Features:
...
- Formatted monospaced ASCII table delivery directly to Discord.
"""
...
COMMON_ALIASES = {"kdb": "de bruyne", "vvd": "van dijk", "trent": "alexander-arnold", "taa": "alexander-arnold", "bruno": "fernandes"}
...
def fetch_data_and_compare():
    ...
    xmins1 = estimate_xmins(p1); ev_1gw_1 = get_ensemble_ev(p1, {}, market_data, weights); macro_4gw_1 = get_macro_ev(p1, team_avg_fdr, weights, {})
    val_1gw_1 = ev_1gw_1 / p1["cost"] if p1["cost"] > 0 else 0.0; val_4gw_1 = macro_4gw_1 / p1["cost"] if p1["cost"] > 0 else 0.0

    xmins2 = estimate_xmins(p2); ev_1gw_2 = get_ensemble_ev(p2, {}, market_data, weights); macro_4gw_2 = get_macro_ev(p2, team_avg_fdr, weights, {})
    val_1gw_2 = ev_1gw_2 / p2["cost"] if p2["cost"] > 0 else 0.0; val_4gw_2 = macro_4gw_2 / p2["cost"] if p2["cost"] > 0 else 0.0

    diff_1gw, diff_4gw = abs(ev_1gw_1 - ev_1gw_2), abs(macro_4gw_1 - macro_4gw_2)
    winner = p1['name'] if macro_4gw_1 > macro_4gw_2 else (p2['name'] if macro_4gw_2 > macro_4gw_1 else "Equal")
    rec = f"{winner} projects higher structural value (+{diff_1gw:.2f} 1-GW EV | +{diff_4gw:.2f} 4-GW xP)."
    if p1["pos"] != p2["pos"]: rec += f" [Cross-Position Audit: {p1['pos']} vs. {p2['pos']}]"

    def fmt_row(lbl, v1, v2): return f"{lbl:<23} | {str(v1):<25} | {str(v2):<25}\n"

    ascii_table = (
        "================================================================================\n"
        + fmt_row("METRIC", f"{p1['name']} ({p1['team']})", f"{p2['name']} ({p2['team']})")
        + "--------------------------------------------------------------------------------\n"
    ...
    requests.post(DISCORD_WEBHOOK_URL, json={"content": f"**[Multi-Model Player Head-to-Head Audit]**\n```text\n{ascii_table}```"})
