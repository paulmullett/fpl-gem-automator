...
def fetch_live_odds():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Odds API returned status {response.status_code}: {response.text}")
...
def compute_implied_metrics(match_event):
    ...
    try:
        lam_total = fsolve(objective, [2.5])[0]
    except:
        lam_total = 2.5
...
def get_market_adjustments():
    ...
    for event in events:
        ...
        if metrics:
            h_full = metrics["home_team"]
            a_full = metrics["away_team"]
            
            h_short = TEAM_NAME_MAP.get(h_full, h_full)
            a_short = TEAM_NAME_MAP.get(a_full, a_full)
