...
- Generate an aligned ASCII Tactical Exploit Diagram inside a `text` codeblock, formatted EXACTLY like this:
+-------------------------------------------------------------------------------+
...
- STRICT NULL-STATE LOGIC: If the LIVE ITK NEWS section contains no quotes, output exactly: 'Status: Awaiting live press conference data.'
...
def get_fpl_data():
    ...
    projected_starting_xP = sum(get_ensemble_ev(p, xmins_overrides, market_data, weights, RISK_POSTURE) for p in starters)
    if cap:
        cap_mult = 2.0 if ACTIVE_CHIP == "TRIPLE_CAPTAIN" else 1.0
        projected_starting_xP += get_ensemble_ev(cap, xmins_overrides, market_data, weights, RISK_POSTURE) * cap_mult
    ...
    locked_squad_str += "================================================================================\n"
    locked_squad_str += f"GW{target_gw} Formation: {formation:<5} | Liquid Value: £{total_liquid_budget:<5.1f}m | Bank: £{bank:<4.1f}m\n"
    locked_squad_str += f"Captain (C): {cap['name'] if cap else 'N/A'} ({cap['team'] if cap else ''}) | Vice-Captain (VC): {vice['name'] if vice else 'N/A'} ({vice['team'] if vice else ''})\n\n"
    locked_squad_str += "STARTING XI:\n"
    for p in starters:
        is_cap = " (C)" if cap and p["id"] == cap["id"] else ""
        is_vice = " (VC)" if vice and p["id"] == vice["id"] else ""
        pid_str = str(p["id"])
        xmins = float(xmins_overrides[pid_str]) if pid_str in xmins_overrides else estimate_xmins(p)
        actual_ev = round(get_base_ev(p, xmins_overrides, weights, market_data), 2)
        pos_str = f"{p['pos']}:"
        name_team = f"{p['name']} ({p['team']})"
        locked_squad_str += f"{pos_str:<4} {name_team:<22} | £{p['cost']:>4.1f}m | {xmins:>4.1f} xMins | {actual_ev:>4.2f} EV{is_cap}{is_vice}\n"
    ...
    for i, p in enumerate(bench_gk + bench_outfield):
        ...
        actual_ev = round(get_base_ev(p, xmins_overrides, weights, market_data), 2)
        name_team = f"{p['name']} ({p['team']})"
        locked_squad_str += f"{prefix:<4} {name_team:<22} | £{p['cost']:>4.1f}m | {xmins:>4.1f} xMins | {actual_ev:>4.2f} EV\n"
        
    locked_squad_str += "\n================================================================================\n"
    locked_squad_str += "                         MULTI-PERIOD TRANSFER TREE\n"
    locked_squad_str += "================================================================================\n"
    if transfer_plan:
        for tp in transfer_plan: locked_squad_str += f"• {tp}\n"
    else:
        locked_squad_str += "• GW1: Hold / Bank Transfer (Unlimited pre-season allocation locked)\n"
    ...
    locked_squad_str += "================================================================================\n"
    for rec in chip_recommendations: locked_squad_str += f"• {rec}\n"
    ...
    locked_squad_str += "================================================================================\n"
    locked_squad_str += f"• 1-GW Expected Yield (Odds-Adjusted): {projected_starting_xP:>6.2f} pts\n"
    locked_squad_str += f"• 8-GW Deep-Tree Horizon xP:           {macro_squad_8gw_xp:>6.2f} pts\n"
