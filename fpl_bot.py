...
- Detail and justify the transfer roadmap generated by the MPO solver across future horizons. 
- Point Hit Constraint: Forbidden from recommending a point hit (-4) unless the 8-Gameweek Expected Value (EV) of the incoming player exceeds the outgoing player by >5.5 points.
...
def get_fpl_data():
    ...
    locked_squad_str += "================================================================================\n"
    locked_squad_str += f" FORMATION: {formation:<6} | BANK: £{bank:<4.1f}m | SQUAD VALUE: £{total_liquid_budget:<5.1f}m\n"
    locked_squad_str += f" CAPTAIN (C): {cap['name'] if cap else 'N/A':<15} | VICE-CAPTAIN (VC): {vice['name'] if vice else 'N/A':<15}\n"
    locked_squad_str += "--------------------------------------------------------------------------------\n"
    ...
    for p in starters:
        ...
        pos_str = f"{p['pos']:<3}"
        name_str = f"{p['name']}{is_cap}{is_vice}:"
        locked_squad_str += f"  {pos_str} | {name_str:<22} ({p['team']:<3}) | £{p['cost']:>4.1f}m | {xmins:>4.1f} xMins | {actual_ev:>4.2f} EV\n"
        
    locked_squad_str += "--------------------------------------------------------------------------------\n"
    locked_squad_str += " BENCH HIERARCHY:\n"
    ...
    for i, p in enumerate(bench_gk + bench_outfield):
        ...
        actual_ev = round(get_base_ev(p, xmins_overrides, weights, market_data), 2)
        name_str = f"{p['name']}:"
        locked_squad_str += f"  {prefix:<3} | {name_str:<22} ({p['team']:<3}) | £{p['cost']:>4.1f}m | {xmins:>4.1f} xMins | {actual_ev:>4.2f} EV\n"
        
    locked_squad_str += "================================================================================\n"
    locked_squad_str += " MULTI-PERIOD TRANSFER TREE (MPO ROADMAP):\n"
    if transfer_plan:
        for tp in transfer_plan: locked_squad_str += f"  • {tp}\n"
    else:
        locked_squad_str += "  • GW1: Lock Core Baseline Structure (Unlimited Free Transfers)\n"
    ...
    locked_squad_str += f"  • 1-GW Expected Yield:         {projected_starting_xP:>6.2f} pts\n"
    locked_squad_str += f"  • 8-GW Deep-Tree Horizon xP:   {macro_squad_8gw_xp:>6.2f} pts\n"
    locked_squad_str += f"  • Stochastic Floor (10th %):   {starter_floor:>6.1f} pts\n"
    locked_squad_str += f"  • Stochastic Ceiling (90th %): {starter_ceiling:>6.1f} pts\n"
    locked_squad_str += "================================================================================\n"
    locked_squad_str += "```\n"
...
def build_prompt(target_gw, bank, free_transfers, locked_squad_str, market_str, new_arrivals_str, live_news):
    ...
    focus_instructions = (
        ...
        f"3. Visual Aesthetics: Single-line bullet points in Section 2. Avoid wide Markdown tables.\n"
        f"4. Analytical Justification: Use EXACT 'TRUE 1-GW EV' numbers provided.\n"
        f"5. MANDATORY SIGN-OFF: Conclude response with boxed 'FINAL LOCKED-IN SQUAD SUMMARY' block exactly as provided."
    )
