...
- Point Hit Constraint: Forbidden from recommending a point hit (-4) unless the 8-Gameweek Expected Value (EV) of the incoming player exceeds the outgoing player by >5.5 points, OR if required to field 11 starters.
...
def get_fpl_data():
    ...
    macro_squad_8gw_xp = sum(get_macro_ev(p, team_avg_fdr, weights, xmins_overrides, market_data, horizons=8) for p in optimal_squad)
    ...
    locked_squad_str += f"• 8-GW Deep-Tree Horizon xP:           {macro_squad_8gw_xp:.2f} pts\n"
